...
public class ModFluids {
    ...
    public static final Map<Acid, FluidRegistryObject<ForgeFlowingFluid>> AGED_ALCOHOL = Helpers.mapOfKeys(Acid.class, fluid -> registerFluid(
            fluid.getId(),
            properties ->
                    properties
                            .block(Registers.ACID.get(fluid))
                            .bucket(Registers.FLUID_BUCKETS.get(fluid)),
            waterLike()
                    .descriptionId("fluid.ktfextras." + fluid.getId())
